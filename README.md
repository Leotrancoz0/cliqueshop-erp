...
O inventário detalhado de responsabilidades e limites públicos está em [Módulos do produto](docs/MODULES.md).
...
O diagrama é intencionalmente abstrato. Endpoints, topologia, contratos, chaves de deduplicação e regras proprietárias não fazem parte da exposição pública. Veja [Arquitetura e limites](docs/ARCHITECTURE.md).
...
Depois, abra <code>http://localhost:8080</code>. Também é possível abrir <code>index.html</code> diretamente. A pasta está pronta para GitHub Pages, mas nenhuma publicação foi realizada.
